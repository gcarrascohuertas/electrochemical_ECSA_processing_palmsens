...
def treatment_csv_rawdata():
    ...
    for file in files: #for every file contained in files

        df = pd.read_csv(file, encoding='utf-16') #generate dataframe of pandas from csv with utf-16 encoding
        ...
        print("Files generated are : " + "CV_generated_file_" +str(n)+".txt") #print files generated

        a=list(df['Voltage'].array)  #convert to list dataframe-data associated to potential 
        b=list(df['Intensity'].array) #convert to list dataframe-data associated to current intensity
        c=np.array(list(zip(a,b))) #merge lists a and b
        df_list.append(b) 
...
def new_generator(color1,label1):
    ...
    fig, ax = plt.subplots() #generate subplots name as ax

  
    for file in files[:1]: #for file contained in files
    ...
    for file in files: #for file contained in files
        f=np.genfromtxt(file, delimiter=" ", skip_header=2) #load file with "tab" delimiter and skip first 2 rows
        x = f[:, xcol] #load data associated to x-column
    ...
    ax.set_ylabel('Current intensity (\u00B5A)')
    ...
    new_array = np.array(list) #generate and array from list 
    mean = np.mean(new_array, axis=0) #generated mean from new_array position 0
    deviation_std=np.std(new_array, axis=0)  #generated standard deviation from new_array position 0
    np.savetxt("data_mean_CV_" + str(concentration) +".txt", mean)  #save mean in .txt file
    np.savetxt("data_dev_std_"+ str(concentration) +".txt", deviation_std)  #save standard deviation in .txt file
...
def plot_mean_std_dev():
    ...
    fig, ax = plt.subplots()  #generate subplots name as ax
    fig, ax2 = plt.subplots()  #generate subplots name as ax2
    ...
    ax.set_xlabel("Potential vs. Ag/AgCl (V) ")   # X-axe limits
    ax.set_ylabel('Current intensity (\u00B5A)')  # Y-axe limits
    ax.set_xlim(-0.1,0.7)  # X-axe limits
    ax.set_ylim(-200,200)  # Y-axe limits
    ...
    ax2.set_xlabel("Potential vs. Ag/AgCl , 3.5 M (V) ")   # X-axe limits
    ax2.set_ylabel('Current density (\u00B5A/cm2)') # Y-axe limits
    ax2.set_xlim(-0.1,0.7)   # X-axe limits
...
def ECSA_reduction_branch():
    ...
    ax.tick_params(axis="y", right=True, direction='in')
    ax.tick_params(axis="x",top=True , direction='in')
    ax.set_xlabel("Potential vs. Ag/AgCl , 3.5 M (V) ") # X-axe limits
    ax.set_ylabel('Current intensity (\u00B5A)') # Y-axe limits
    ax.set_xlim(-0.1,0.7)  # X-axe limits
    ax.set_ylim(-200,200)  # Y-axe limits
...
def move_data():
    ...
    files2 = glob.glob(path+ "/CV_generated_file_*.txt")  #list all the files  named as "CV_generated_file_" with extensión .txt
    files3 = glob.glob(path+ "/data_*.txt") #list all the files  named as "data" with extensión .txt
    ...
    graphs1 = glob.glob(path+ "/figure*.png") #list all the files  named as "figure" with extensión .png
    print("Files  moved to directories are: "+str(graphs1)) 
    graphs2 = glob.glob(path+ "/figure*.eps") #list all the files  named as "figure" with extensión .eps
